...
仓库地址：<https://github.com/VincentZyuApps/tampermonkey-plugin-koishi-market-ai-helper>
...
4. GitHub Actions 会读取 `package.json`，默认生成 `vX.Y.Z` Release tag。
...
| `npm version X.Y.Z --no-git-tag-version` | 指定精确版本号，例如 `npm version 0.2.2 --no-git-tag-version` |

`patch` 不是占位符，不能写成指定版本；它表示“按 semver 规则递增 patch 位”。

如果要指定版本号，直接把 `X.Y.Z` 换成目标版本号。
...
`build publish` 会在 Release 和 Pages 完成后尝试通知 Greasy Fork webhook。

如果需要启用这个通知，在 GitHub 仓库中配置 Actions secret：

```text
GREASYFORK_WEBHOOK_URL
```

如果没有配置该 secret，workflow 会跳过 Greasy Fork 通知，不会失败。
